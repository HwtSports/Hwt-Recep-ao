...
<head>
    ...
    <title>Recepção HWT - Sistema Hoteleiro Sincronizado</title>
    <meta name="description" content="Sistema gratuito de gestão hoteleira para pequenos hotéis e pousadas com sincronização em tempo real">
...
<body>
    ...
    <script>
        ...
        function verificarMudancas() {
            ...
            const novaAtualizacao = localStorage.getItem('ultimaAtualizacaoHWT');
            
            // Se houve uma atualização desde a última verificação
            if (novaAtualizacao && novaAtualizacao !== ultimaAtualizacao) {
                sincronizando = true;
                atualizarIndicadorSincronizacao('sincronizando');
                
                // Carregar dados atualizados
                const novosHospedes = JSON.parse(localStorage.getItem('hospedesAtivos')) || [];
                const novoHistorico = JSON.parse(localStorage.getItem('historico')) || [];
                
                // Verificar se houve mudanças
                if (JSON.stringify(novosHospedes) !== JSON.stringify(hospedesAtivos) || 
                    JSON.stringify(novoHistorico) !== JSON.stringify(historico)) {
                    
                    hospedesAtivos = novosHospedes;
                    historico = novoHistorico;
                    ultimaAtualizacao = novaAtualizacao;
        ...
        function atualizarIndicadorSincronizacao(status = 'sincronizado') {
            ...
            switch(status) {
                ...
                case 'erro':
                    ...
                    textElement.textContent = 'Erro de sincronização';
                    break;
            }
        }
        ...
        function liberarQuartoAutomaticamente(quarto, index) {
            ...
            // Salvar dados e notificar mudança
            salvarDados();
        ...
        function salvarDados() {
            ...
            // Marcar a última atualização para sincronização
